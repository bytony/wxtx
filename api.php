...
{     
	...
	if(!empty($fromUsername) && !empty($toUsername))
	{
		...
		$uid=$uinfo['id'];
		$randKeywords['content']="http://127.0.0.1/index.php?con=wap&act=index&id=".$uinfo['id'];
...
function parseStr($info,$type,$flag=0)
{
	...
	elseif($type=='news')
	{
		...
		{
			$url=!empty($einfo['url'])?$einfo['url']:'http://127.0.0.1/?con=wap&act=item&id='.$einfo['id'];
	...
	elseif($type=='lbs')
	{
		...
		{
			$url='http://127.0.0.1/?pluginid=lbs&con=wap&act=item&id='.$einfo['id'];
			$einfo['url']=createUrl($url);
