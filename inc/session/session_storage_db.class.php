...
class session_storage_db{
	...
	var $_sessionTable = 'bidcms_session';
	
	/**
	 * 构造函数
	 */
	function session_storage_db($db){
		$this->_db = $db;
	}
	...
	function read($id){
		...
		$result = $this->_db->result_first( 'SELECT `data` FROM '.$this->_sessionTable. 
												" WHERE `sessionid` = '{$id}' " 
											);
	...
	function write($id, $sess_data){
		...
		$this->_db->query('REPLACE INTO '.$this->_sessionTable. 
	...
	function destroy($id){
		$id = mysql_real_escape_string($id);
		$this->_db->query('DELETE FROM '. $this->_sessionTable. 
							" WHERE `sessionid` = '{$id}' "
							);
	}
