...
class session_operator_simulator{
	...
	function session_regenerate_id( $delete_old_session = false ){
		...
		if( function_exists('dsetcookie') ){
			dsetcookie(BIDCMS_CLIENT_SESSION, $session_id_cookie, 0);
		}else{
			setcookie(BIDCMS_CLIENT_SESSION, $session_id_cookie, 0);
		}
		return true;
	}
	...
	function session_start(){
		global $_G;
		$session_id = '';
		if ( isset($_G['cookie'][BIDCMS_CLIENT_SESSION]) ){
			$session_id = (string)$_G['cookie'][BIDCMS_CLIENT_SESSION];
		}
